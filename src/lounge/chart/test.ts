...
import { createStatsConfig, createStatsData } from './stats.ts'
import { createDeltaConfig } from './delta.ts'
...
Deno.test('stats', async () => {
    ...
    new Chart(canvas.getContext('2d'), createStatsConfig({
        data: createStatsData(details.mmrChanges.reverse()),
...
Deno.test('delta', async () => {
    ...
    const canvas = createCanvas(1280, 720)
    new Chart(canvas.getContext('2d'), createDeltaConfig({
